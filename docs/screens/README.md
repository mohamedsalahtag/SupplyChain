...
| 23 | PO preparation, PO draft and the SAP outbox (Stage 7): SKU picking and split, master-data requests, build / validate / submit, unknown outcomes reconciled by reference, manual resolution with evidence, SAP simulator with fault injection, Configuration → SAP purchase orders (PO API) | Built · waiting for acceptance |
| 24 | Reports and KPIs (Stage 8): demand execution by business origin (drawer per demand: lines and containers per week), change request register, performance (rates, stage times, on time, CR / ack / handoff / SAP KPIs), Excel export | Built · waiting for acceptance |
