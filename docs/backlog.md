...
| 19 | Supplier view as a real Excel file (.xlsx) instead of CSV — needs an Excel library (new dependency) | 2026-09-24 | 18 |
...
| 26 | **Required before production:** the real SAP (ZCON) PO adapter, and the SAP field that stores the portal reference (POD-…) so unknown outcomes can be found by reference. Run the Stage 7 fault suite against the SAP test system | 2026-09-25 | 23 |
...
| 32 | Security follow-ups (review 2026-09): HTTPS proxy + built bundle on the server; session revocation (session version on `app.User`); login rate limit per username; authenticate uploads before reading the body; malware scan of attachments; Helmet/CSP; SAP purchase-order history scoped by company | 2026-09-25 | — |
